...
private:
    unsigned char m_duration_of_pregnancy;
    unsigned char m_young_in_litter;
...
m_duration_of_pregnancy(_duration_of_pregnancy),m_young_in_litter(_young_in_litter){}
...
inline std::ostream& operator<<(std::ostream& os, const Mammals& an) {
    std::cout<<(Animal*)&an;
    std::cout<< "duration of pregnancy: "<<+an.m_duration_of_pregnancy<<"\nnumber of young born in each litter: "<<+an.m_young_in_litter<<std::endl;
    return os;
}
#endif //ZOO_MAMMALS_H
